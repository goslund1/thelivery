...
- **Tune Suggestion Viewer** (`SuggestionViewer.vue`) — admin-only panel (launched from Filters flyout badge count). Fetches all suggestions via `GET /api/admin/suggestions`. Two tabs: Pending / Liked. Dropdown selector cycles between suggestions on the active tab; auto-advances to the next entry after Dismiss or Like (which moves the card to the other tab). Actions: Like (toggleable; moves pending → liked), Promote (calls `cardsStore.promoteCard()` to fork the card with the suggested adjustments), Dismiss (removes from list).
  ...
  **Bg scroll-to-card (pending implementation):** The intent is that switching suggestions scrolls the background page so the associated card's slideshow is visible behind the glass. The anchor already exists: `CardView.vue` renders `<div class="card" :id="\`card-${card.id}\`">` (line 79). Implementation: `watch(current, () => { const el = document.getElementById(\`card-${current.value.cardId}\`); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' }) })` in SuggestionViewer. The offset concern: `block: 'start'` lands the card at the top of the viewport, but the panel may overlap it — consider `window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - <offset>, behavior: 'smooth' })` with an offset tuned to clear the panel. Do **not** use `block: 'nearest'` — that walks the full scroll chain and caused the thumb-rail page-jump bug (see Slideshow section above).
